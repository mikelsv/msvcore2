...
#define ppexm(tmem, umem, nmem, buf) {while(tmem<umem+nmem) { tmem*=2; BYTE*b=(unsigned char*)malloc(tmem); memcpy(b, buf, umem); free(buf); buf=b; } }
...
public:
	...
	MSVEF(){
		...
		code=(unsigned char*)malloc(_codesz); stack=(unsigned char*)malloc(_stacksz); ret=(unsigned char*)malloc(_retsz);
		fcmp=0;
	}
	~MSVEF(){ Clear(); }
...
//	MSVEFB ef=data[i];
	switch(code[codel]){
		...
		case EFC_FIND: codel++;
			...
			t.Reserve(tmp); memcpy(t, &code[codel], tmp); codel+=tmp; EFLOG("FIND:"+t);
			if(p==-1) p=line.size();
...
//			else push(stack, _stacksz, stacksz, &p, 4);
		...
		case EFO_RET: codel++; 
			tmp=*(unsigned int*)&code[codel]; codel+=4;
			efretn.Reserve(tmp); memcpy(efretn, &code[codel], tmp); codel+=tmp; efret=1;
		break;
...
if(line.compare((char*)ret+pos, i)){ pos+=i; if(pos>=_retsz) return MString();
	i=*(unsigned int*)(ret+pos); pos+=4; retd.Reserve(i); memcpy(retd, ret+pos, i); pos+=i; return retd;
...
void DelRet(){
if (_stacksz!=def_stacksz) {free(stack); stack=(unsigned char*)malloc(_stacksz); } memset(stack, 0, _stacksz);
if (_retsz!=def_retsz) {free(ret); ret=(unsigned char*)malloc(_retsz); } memset(ret, 0, _retsz);
retsz=0; stacksz=0;
return ;}
...
if (_stacksz!=def_stacksz) {free(stack); stack=(unsigned char*)malloc(_stacksz); } memset(stack, 0, _stacksz);
if (_retsz!=def_retsz) {free(ret); ret=(unsigned char*)malloc(_retsz); } memset(ret, 0, _retsz);
if (_codesz!=def_codesz) {free(code); code=(unsigned char*)malloc(_codesz); } memset(code, 0, _codesz);
...
void Clear(){
	free(stack); stack = 0; _stacksz=0;
	free(ret); ret = 0; _retsz=0;
	free(code); code = 0; _codesz=0;
}
