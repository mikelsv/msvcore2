...
void New(unsigned int size, unsigned int base, unsigned int basesize){
	free(data); sz=size; data=(unsigned int*)malloc((sz+base*2)*sizeof(int)+sz*basesize);
...
void myfastsort_VSiLine(IList<VSi> &list){
	...
	unsigned int base = 256, bits = mfsbs(base - 1), k, p;
	mfsstack st;
	unsigned int j; 
	
	st.New(size, base, sizeof(VSi));

	unsigned int maxs = 0;
	for(unsigned int i = 0; i < size; i ++){
		if(maxs < arr[i].key.size())
			maxs = arr[i].key;
	}

for(unsigned int i = maxs - 1; i >= 0; i --){
	memset(st.data, 0, size*sizeof(int)+size*sizeof(VSi)); memset(st.fpos, 255, base*2*sizeof(int));
  //  bit=(base-1)<<i;
    for(j=0; j<size; j++){
        k=upd(arr[j].key[i]); // k=k>>i;
