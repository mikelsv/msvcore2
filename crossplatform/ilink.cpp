...
ILink::ILink(const VString link, int type){
	Clean();
	Link(link, type);
}

ILink::ILink(const ILink &ilink){
	Clean();
}
...
mdata.Add(_proto, VString(_domain.rchar(), _domain.size()+portsz), _path, _file, "?", _iquest, "#", _target, ilink.size()>sizeof(DUVS)*args ? ilink.str(0, sizeof(DUVS)*args) : mdata.Reserve(sizeof(DUVS)*args)); // , _name, _ext
	//ln=mdata;
...
if(mv>0){ mdata.Reserve(mdata.size()+mv+(mdata.size()==0)); }
...
if(mv>0){ mdata.Reserve(mdata.size()+mv+(mdata.size()==0)+(args-ar)*sizeof(DUVS)); }
char*mvp=mdata.rchar()+vp;
