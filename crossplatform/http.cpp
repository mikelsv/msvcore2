...
struct httpval{
	...
	httpval* Insert(LString &data, httpval *&v, const VString &key, const VString &val){
...
public:
	...
	void Set(VString line, VString _domain, VString _path){
		...
		if(domain && path && name)
			cookies.NewE()->Set(domain, path, name, value, expires);
...
public:
	...
	int64 GetRecvSz(){ return arecv; }
	int64 GetSendSz(){ return asend; }
	
protected:
	int DoRequest(VString url){
		LString ls; ILink il; int snd;
		arecv = 0; asend = 0;
