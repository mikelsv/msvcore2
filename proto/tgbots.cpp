...
public:
	...
	int TestBot(){
		...
		// http request
		gp.Request(LString() + "https://api.telegram.org/bot" + token + "/getMe");
	...
	int InitBot(){
		...
		gp.Request(LString() + "https://api.telegram.org/bot" + token + "/setWebhook?url=" + bot_url 
...
public:
	...
	int LoadBots(VString path = "bots/"){
		Readdir rd;
		rd.ReadDir(LString() + path + "*");

		for(unsigned int i = 0; i < rd.size(); i ++){
			if(!rd[i].isdir()){
				ConfLine cf(LString() + path + rd[i].key);

				if(!cf["token"])
					continue;

				TgBot *p = bots.NewE();
				p->Load(LString() + path + rd[i].key, this);
	...
	int Call(VString head, VString post){
		...
		if(message && text){
			...
			if(text[0] == '/')
				CallCmd(bot, ct, chat_id, text.str(1));
			else
				return CallMsg(bot, ct, chat_id, text);
	...
	// Analys user command
	virtual int CallCmd(TgBot *bot, XDataCont &ct, VString id, VString cmd){
		if(cmd == "start"){
	...
	// Analys message
	virtual int CallMsg(TgBot *bot, XDataCont &ct, VString id, VString text){
	...
	int SendMsg(TgBot *bot, VString id, VString text, VString reply_to = VString()){
		GetHttp gp;

		LString req;
		
		req + "https://api.telegram.org/bot" + bot->token + "/sendMessage?chat_id=" + id + "&text=" + HtiGetEncode(text);

		if(reply_to)
			req + "&reply_to_message_id=" + reply_to;		

		return gp.Request(req);
	}
	...
	VString GetRandomText(){
		return botword[rand() % botword.Size()].key;
	}
