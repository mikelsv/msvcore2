...
TString WebSocketAcceptKey(VString key){
	//MString ret;
	...
	//TString t = Base64::stob(htob(btoh(mh.Get())));
...
int WebSocketDecodeData(VString read, int &r_opcode, VString &r_value){
		...
		switch(len&127){
			case 126:
				len = htons(*(unsigned short*)(read.data + 2));
				lpos = 4;
			break;
			case 127:
				if(read.sz<10)
					return -1;
				len = htonll(*(uint64*)(read.data + 2));
				lpos = 10;
			break;
			default:
				lpos = 2;
...
public:
	...
	// operations
	int Connect(VString path, VString host){
		...
		return SendHello(host);
	}

	int SendHello(VString host){
		SString it;
	...
	int Send(VString line, int enc = 1){
		...
		MString sline;
		if(enc)
			line = sline = WebSocketEncodeData(LWSOC_STRING, line);
