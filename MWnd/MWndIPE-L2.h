...
//	int SetDC(HDC d){ dc=d; }
	...
	int SetPixel(int x, int y, unsigned int c){
		if(x < 0 || y < 0 || x >= width || y >= height)
			return 0;
	...
	int GetNormalPixel(int x, int y){
		if(x < 0 || y < 0 || x >= width || y >= height)
			return 0;
		...
		return (*(int*)(GetData() + x * 3 + y * ewidth))&16777215;
#else
		return (*(int*)(GetData() + x * 3 + (height - y - 1) * ewidth))&16777215;
#endif
	}
...
int DrawPoly(DINT *a, int sz, /*int x, int y, */ MWndStyle style, bool cp=0){
	...
	int sx=pr-pl+1; it.Reserve(rx+2); buf=it.data;
...
MString bf; char bfc[128], *b; int bs=pr-pl+1; if(bs>128) { bf.Reserve(bs, 0); b=bf; } else {b=bfc;}
...
int SaveToMem(TString &ret){
	// Reserv memory
	if(!ret.Reserve(14 + sizeof(BITMAPINFOHEADER) + dsize))
		return 0;
