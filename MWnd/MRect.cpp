...
public:
	...
	void SetB(int _x, int _y, int _bx, int _by){
		x = _x;
		y = _y;
		sx = 0;
		sy = 0;
...
//	bool isext;	
	...
	MExRect(const RECT &rc){
		left=rc.left; top=rc.top; right=rc.right; bottom=rc.bottom; set=0;
	}
...
/*		bool Set(MString rect, int rx=0, int ry=0){Null();
	...
	operator SRect()const {SRect sr(l(0), t(0), r(0) - l(0), b(0) - t(0)); return sr;}
...
class SExRect{
public:
	int x, y, tx, ty, rx, ry, opt;
 
	SExRect(){
		Clean();
	}

	// values: (absolute) int, (rx or ry) -int, int%.
	SExRect(VString rect, const int rx = 0, const int ry = 0){
		Set(rect, rx, ry);
	}

	//SExRect(const MRect &rect){Set(rect.left, rect.top, rect.right, rect.bottom);}
	//SExRect(VString rect, const int rx=0, const int ry=0){ Set(rect, rx, ry); }
	///SExRect(const char *rect, const int rx=0, const int ry=0){ Set(rect, rx, ry); }
	//SExRect(int _left, int _top, int _right, int _bottom, BYTE p=0){Null(); left=_left; top=_top; right=_right; bottom=_bottom; set=p;}
	

bool Set(VString rect, const int nrx =  0, const int nry = 0){
		Clean();
	...
	int l()const {
		return opt&1 ? (opt&2 ? (100-x)*rx/100 : rx-x) : (opt&2 ? (x)*rx/100 : x);
	}

	int t()const {
		return opt&4 ? (opt&8 ? (100-y)*ry/100 : ry-y) : (opt&8 ? (y)*ry/100 : y);
	}

	int r()const {
		return opt&16 ? (opt&32 ? (100-tx)*rx/100 : rx-tx) : (opt&32 ? (tx)*rx/100 : tx);
	}

	int b()const {
		return opt&64 ? (opt&128 ? (100-ty)*ry/100 : ry-ty) : (opt&128 ? (ty)*ry/100 : ty);
	}
	...
	int GetH(const int ry)const{
		return b() - t();
	}

	operator SRect()const {
		SRect sr(l(), t(), r() - l(), b() - t());
		return sr;
	}
...
public:
	...
	void SetFont(VString font, LONG h=0){
		...
		for(unsigned int i=0; i<font.sz; i++){
			lfFaceName[i]=font[i];
		}
