...
public:
	...
	virtual bool OnLButtonDown(unsigned int keys, unsigned short cx, unsigned short cy){ mouse.ld = 1; mouse.x = cx; mouse.y = cy; return 0; }
	virtual bool OnLButtonUp(unsigned int keys, unsigned short cx, unsigned short cy){ mouse.ld = 0; return 0; }
	virtual bool OnMouseMove(unsigned int keys, unsigned short cx, unsigned short cy){
		...
		//if(mouse.x > map_sx - rect.GetW())
		//	mouse.x = map_sx - rect.GetW();
...
class GameObject: public GameBox, public OList<GameObjectD>{
...
public:
	...
	GameObjectD* GoSet(int gid, int id, const SRect &rc, int z=0){
		...
		GameObjectD *p=Search(GameObjectFindById, gof);
		if(!p) p=NewE();
...
//	}
	...
	// Clear
	void Clear(){ OList<GameObjectD>::Clean(); }
