...
public:
	...
	int SetData(int chars, int dsz, unsigned char *line, unsigned int *pos){
		_data.Reserve(dsz+chars*sizeof(void*));
...
// 4,6,7,8,9,10,11,12,14,16,18,20,22,24,24,30,36,48,72
	...
	mf._data.Reserve(szsz*sizeof(MyFontF), 0); mf.sz=szsz; mf.font=(MyFontF*)mf._data.rchar();// font.lfWeight=0;

	for(int ii=0; ii<szsz; ii++){
	MyFontF*mff=&mf.font[ii]; sa=0;
	mff->sz=fval; mff->tsz=tval; mf.bl.set(szs[ii], 1); mf.pos[szs[ii]]=ii;
	MString data; data.Reserve((tval-fval)*(2+4)); mff->szx=(BYTE*)data.rchar(); mff->szy=(BYTE*)data.pos(tval-fval); mff->data=(int*)data.pos((tval-fval)*2);
	font.lfHeight=szs[ii];
...
mff->_data.Reserve((2+4)*(tval-fval)+sa/8, 0); memcpy(mff->_data, data, data);
mff->szx=(BYTE*)mff->_data.rchar(); mff->szy=(BYTE*)mff->_data.pos(tval-fval); mff->data=(int*)mff->_data.pos((tval-fval)*2);
...
mf._data.Reserve(mf.sz*sizeof(MyFontF), 0); mf.font=(MyFontF*)mf._data.rchar();
