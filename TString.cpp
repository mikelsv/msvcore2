...
//#define ThreadStringUListDataSize(el) ((char*)el->&data - (char*)&el)
...
void ThreadStringCore::Init(){
	if(!do_tstring_core){
		...
		msvcorenew(do_tstring_core, ThreadStringCore);
	}
	return ;
}
...
void ThreadStringDest(){
	...
	msvcoredelete(do_tstring_core, ThreadStringCore);
	do_tstring_core = 0;
#endif
...
TString& TString::Reserve(int size, int p){
	if(size == sz)
		return *this;
