...
public:
	...
	virtual void AnalysXmlLineServ(storm_work_el &wel, XDataEl *el){
		...
		if(act == "stream:stream"){
			...
			if(!el->GetLVal("type")){
				...
				it.Format("<db:verify from='%s' to='%s'>%s</db:verify>", to, from, el->a()->v());
				...
				if(con = XmppSend(to, from, it)){
					...
					XmppConnect(to, from, this, el->a()->v());
			}
			...
			if(!el->GetLVal("type")){
				...
				int ver = el->a()->v() == XmppGenerateVerifyCode(to, from);
				if(ver)
	...
	virtual void OnMsg(XDataEl *el){}
	virtual void OnRoomOpen(XDataEl *el){}
