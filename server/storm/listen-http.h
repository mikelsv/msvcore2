...
/* Bad code
		...
		if(!rootpath)
			rootpath = msvcorestate.procpath;
...
//				int f = pcre2_match(p, rurl, rurl, 0, 0, arr, 255);
		...
		if(rootpath[0] != '/' && rootpath[1] != ':' && rootpath[0] != '\\'){
			itroot.Add(msvcorestate.procpath, "/", rootpath);
			itroot.Add(NormalHttpPath(itroot), VString());
			ilink.Link(it2.Add(/*_procpath, "/", rootpath,*/ itroot, "/", NormalHttpPath(it)), 1);
		} else{
			itroot = rootpath;
			ilink.Link(it2.Add(rootpath, "/", NormalHttpPath(it)), 1);
		}
...
//		}
	...
	void SendHeader(storm_work_el &wel, listen_http_sendstatus &status, VString sendhead, int64 datasize){ // ConfLineOption *host, 
		...
		AnalysSendWriteLog(wel, status, it.sz + datasize); // host, 
