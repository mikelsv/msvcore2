...
// --------------------------------------------------- Set	Установка
VString& VString::operator=(const VString line){
	data = line.data;
	sz = line.sz;
	return *this;
}
...
VString::operator unsigned int()const{ if(data) return sz; return 0; }
...
bool VString::morecompareu(const VString& string)const{
	...
	for (int i=0; i<size; i++){
		...
		else if(upd(data[i])<upd(string.data[i])) return 0;
	}
	return sz>string.sz ? 1 : 0;
}
...
int ParamLine(VString line, VString &name, unsigned char *k, VString *v, int sz){
	...
	while(ln<to+1){
		if(ln==to || *ln=='.' || *ln==',' || *ln=='=' || *ln=='!' || *ln=='*' || *ln=='/' || *ln=='+'/* || *ln=='-'*/ || *ln=='~' || *ln==':'){
