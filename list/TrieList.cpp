...
class TrieList : private AListAllocOList<TrieListEl>, public TrieListEl{
...
//		_e = 0;
	...
	TrieListEl* SetOne(unsigned int k, TrieListEl *parent, int isup = 0){
		...
		TrieListEl *p = parent->_l, *el, *nel = 0;

		if(!p || k < p->k){
			el = AllocNew();
			OMatrixTemplateAddP(parent->_l, parent->_r, nel, el);
			el->k = k;
			el->u = isup;
			return el;
		}
		...
		el = AllocNew();
		OMatrixTemplateAddP(parent->_l, parent->_r, p, el);
		el->k = k;
		el->u = isup;
