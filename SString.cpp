...
template<int stacksize>
const SStringX<stacksize>& SStringX<stacksize>::operator=(const SStringX& line){ unsigned char *odata = data; sz = line; StringNew(line); StringDel(odata); return *this; }
...
SStringX<stacksize>& SStringX<stacksize>::Format(VString line, MTEVA v1, MTEVA v2, MTEVA v3, MTEVA v4, MTEVA v5,
		...
		if(!Reserve((unsigned int)(ls-(unsigned char*)0)))
			return *this;
...
VString SStringX<stacksize>::itos(int64 val, int radix, int null){
	Reserve(prmf_itos(0, 0, val, radix, null));
...
VString SStringX<stacksize>::ipitos(unsigned int val){
	...
	sz+=3;
	Reserve(sz); unsigned char *p = data;
...
VString SStringX<stacksize>::ip6itos(IP6_ADDRESS ip){ // 1:2:3:4:5:6:7:8
	...
	Reserve((unsigned int)((char*)l-(char*)bdata));
...
template<int stacksize>
SStringX<stacksize>& SStringX<stacksize>::Reserve(int size, int p){
	if(size == sz)
		return *this;
